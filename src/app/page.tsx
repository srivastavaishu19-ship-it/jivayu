            ...
            {medicines.length > 0 && (
              <div style={{ marginBottom: '20px' }}>
                <div style={{ fontSize: '11px', fontWeight: '700', color: t.text2, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button onClick={() => markStatus(m.id, 'missed')} style={{flex:1, background: medStatus[m.id]==='missed' ? '#dc2626' : 'rgba(239,68,68,0.1)', color: medStatus[m.id]==='missed' ? '#fff' : t.danger, border:'1px solid rgba(239,68,68,0.3)', borderRadius:'8px', padding:'9px', fontSize:'12px', cursor:'pointer', fontWeight:'700', fontFamily:'inherit', transition:'all 0.2s'}}>{medStatus[m.id]==='missed' ? '✗ Missed!' : '✗ Missed'}</button>
                    </div>
                  </div>
                ))}
